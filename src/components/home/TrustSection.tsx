...
const TrustSection = () => {
  ...
  const badgeName = useCMSValue('trust_badge_name', 'TrustLocal');
  const badgeScore = useCMSValue('trust_badge_score', '9,0');
  const badgeMaxScore = useCMSValue('trust_badge_max_score', '10');
  const badgeReviews = useCMSValue('trust_badge_reviews', '10');
  const badgeUrl = useCMSValue('trust_badge_url', 'https://trustlocal.es/madrid/parla/diseno-web/isla-cloud-solutions/');
  const badgeStars = useCMSValue('trust_badge_stars', '4.5');
  const badgeDescription = useCMSValue('trust_badge_description', 'Diseñadores web en Parla');
            ...
            >
              ...
              <div>
                ...
                <div className="flex items-baseline gap-1 mt-1">
                  <span className="text-3xl font-heading font-bold text-foreground">{badgeScore}</span>
                  <span className="text-sm text-muted-foreground">/{badgeMaxScore}</span>
                </div>
                <div className="flex gap-0.5 mt-1">{renderStars(parseFloat(badgeStars))}</div>
                <span className="text-xs text-muted-foreground mt-1 inline-flex items-center gap-1">
                  {badgeReviews} reseñas <ExternalLink size={10} className="opacity-0 group-hover:opacity-100 transition-opacity" />
                </span>
              </div>
            </motion.a>
          </div>
