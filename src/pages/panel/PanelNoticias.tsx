...
const PanelNoticias = () => {
  ...
  const handleUploadImage = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploading(true);
    try {
      const result = await uploadImage(file, token, 'noticias');
          ...
          {paged.map((n) => {
            ...
            return (
              ...
              >
                ...
                {n.image_url ? (
                  <div className="w-12 h-10 rounded-lg bg-transparent shrink-0 flex items-center justify-center p-1">
                    <img src={n.image_url} alt="" className="w-full h-full object-contain" />
                  </div>
                ) : (
