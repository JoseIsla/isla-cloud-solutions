...
import { partnersApi, uploadImage, type PartnerFromAPI, API_BASE_URL } from '@/lib/api';
import { Button } from '@/components/ui/button';
import { Plus, Pencil, Trash2, X, Upload, Handshake, GripVertical, Search, ChevronUp, ChevronDown, ChevronsUp, ChevronsDown } from 'lucide-react';
...
const PanelPartners = () => {
  ...
  const [editing, setEditing] = useState<Partial<PartnerFromAPI> | null>(null);
  const [isNew, setIsNew] = useState(false);
  const [uploading, setUploading] = useState(false);
  const [mediaPicker, setMediaPicker] = useState(false);
  const fileInputRef = useRef<HTMLInputElement>(null);

  const handleReorder = useCallback(async (reordered: PartnerFromAPI[]) => {
    if (!token) return;
    try {
      await Promise.all(reordered.map((p, i) => partnersApi.update(p.id, { ...p, sort_order: i }, token)));
  ...
  const { getDragProps, isDragOver } = useDragReorder({ items: partners, setItems: setPartners, onReorder: handleReorder });

  const [filter, setFilter] = useState('');

  const load = () => { partnersApi.list(token).then(setPartners).catch(() => {}); };
  useEffect(load, []);

  const moveItem = async (idx: number, dir: -1 | 1) => {
    const newIdx = idx + dir;
    if (newIdx < 0 || newIdx >= partners.length) return;
  ...
  const moveToEdge = async (idx: number, target: 'first' | 'last') => {
    if (idx === (target === 'first' ? 0 : partners.length - 1)) return;
    const reordered = [...partners];
  ...
  const handleSave = async () => {
    ...
    try {
      if (isNew) {
        await partnersApi.create(editing, token);
        toast.success('Partner creado');
      } else {
        await partnersApi.update(editing.id!, editing, token);
  ...
  const handleDelete = async (id: number) => {
    if (!token || !(await confirm('¿Eliminar este partner?', 'Se eliminará permanentemente este partner.'))) return;
    try {
  ...
  const handleUploadLogo = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploading(true);
    try {
      const result = await uploadImage(file, token, 'partners');
    ...
    <PanelLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-xl font-heading font-bold text-foreground">Partners</h2>
            <p className="text-muted-foreground text-sm mt-0.5">{partners.length} partners · <span className="text-muted-foreground/60">Arrastra para reordenar</span></p>
          </div>
          <Button size="sm" onClick={() => { setEditing({ name: '', logo_url: '', website_url: '', sort_order: partners.length }); setIsNew(true); }}>
            <Plus size={16} /> Nuevo
          </Button>
        </div>
          ...
          <div className="fixed inset-0 z-[9999] bg-black/40 backdrop-blur-sm overflow-y-auto flex items-start justify-center p-4 min-h-screen">
            <div className="bg-card rounded-2xl border border-border p-6 w-full max-w-md shadow-2xl my-8">
              <div className="flex justify-between items-center mb-5">
                <h3 className="font-heading font-semibold text-base">{isNew ? 'Nuevo partner' : 'Editar partner'}</h3>
          ...
          {paged.map((p) => {
            ...
            return (
              ...
              >
                ...
                {p.logo_url ? (
                  <img src={p.logo_url} alt={p.name} className="w-9 h-9 rounded-lg object-contain p-1 shrink-0" />
                ) : (
                ...
                {!filter && (
                  <div className="flex items-center gap-0.5 shrink-0">
                    ...
                    <button onClick={() => moveItem(realIdx, 1)} disabled={realIdx === partners.length - 1} className="p-0.5 rounded hover:bg-muted disabled:opacity-20 text-muted-foreground"><ChevronDown size={14} /></button>
                    <button onClick={() => moveToEdge(realIdx, 'last')} disabled={realIdx === partners.length - 1} className="p-0.5 rounded hover:bg-muted disabled:opacity-20 text-muted-foreground" title="Mover al final"><ChevronsDown size={14} /></button>
                  </div>
                )}
                <div className="flex gap-0.5 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => { setEditing(p); setIsNew(false); }} className="p-1.5 rounded-lg hover:bg-primary/10 text-primary"><Pencil size={13} /></button>
                  <button onClick={() => handleDelete(p.id)} className="p-1.5 rounded-lg hover:bg-destructive/10 text-destructive"><Trash2 size={13} /></button>
                </div>
              </StaggerItem>
            );
          })}
